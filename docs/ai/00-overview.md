...
給 Windows 10/11 使用者、把 iPhone 照片與影片備份到電腦的桌面工具。
...
- iPhone 端設定（**設定 → App → 照片 → 傳送到 Mac 或 PC**，截圖見 `docs/img/iPhone_app_setting.jpg`）：
  - **自動** — 手機即時轉檔，出 `.jpg` / `.mov`，較慢
  - **保留原始檔** — 直出 `.heic` / `.heif` / `.mov`
  - 改設定後**必須重新插拔 USB** 才生效
- 目前不處理轉檔（HEIC → JPG 等），交給 iPhone 端設定決定。
- **★「自動」模式的即時轉檔會讓複製速度掉到 0 B/s**（實測），影片尤其明顯。
  這不是我們能修的（轉檔在手機上發生），但必須在 UI 與 README 講清楚，
  否則使用者會以為工具壞了。詳見 `04-shell-com-notes.md`。
