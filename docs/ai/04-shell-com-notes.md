...
### ★ 仍未回答：MTP 的 enumerator 是不是串流

早退（`folder_has_media`）有沒有意義，取決於
**第一次 `Next()` 是串流回傳、還是把整份清單備妥**。

現有數據**無法回答**：
- 量到的三個資料夾各只有 1~3 個檔案，早退無從發揮。
- 而且量測有冷熱順序偏差 —— 先跑早退（冷，要開該資料夾的 MTP 物件）
  再跑完整列舉（熱），導致「早退省下 -102%」這種假象。
  **不要採信那三組數字。**

→ `tools/bench_enum.py` 專門測這件事：取前 1 / 10 / 50 / 200 / 800 筆各要多久。
  若「取 1 筆」≈「取全部」就不是串流，早退無意義；
  若耗時隨筆數線性成長就是串流，早退有意義。
  **所有量測都先 warm-up 再計時**，避免重蹈冷熱偏差。
